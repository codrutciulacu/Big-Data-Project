...
# -----------------------------
# SQL queries (same as your script)
# -----------------------------
SQL_LATEST_ROWS = """
WITH latest AS (
...
# -----------------------------
@dag(
    dag_id="export_route_suggestions_json",
    start_date=days_ago(1),
    schedule="@hourly",
    catchup=False,
    tags=["exports", "json", "gtfs"],
)
def export_route_suggestions_json():
    @task
    def export_all() -> Dict[str, str]:
        ...
        try:
            # Latest route rows
            df_latest = pd.read_sql_query(SQL_LATEST_ROWS, conn)
            route_rows = build_route_rows(df_latest)

            # Write full
            out_full = EXPORT_BASE_DIR / "route_suggestions_latest.json"
            write_json(out_full, route_rows, pretty=pretty)

            # Needs data
            needs_data = [x for x in route_rows if x["suggestion"]["action"] == "needs_data"]
            out_needs = EXPORT_BASE_DIR / "needs_data_latest.json"
            ...
            out_top = EXPORT_BASE_DIR / "top_priority_latest.json"
            ...
            out_summary = EXPORT_BASE_DIR / "summary_latest.json"
            write_json(out_summary, summary, pretty=pretty)

            # Peak hours (optional)
            try:
                df_peak = pd.read_sql_query(SQL_PEAK_HOURS_LATEST, conn)
                peak_payload = df_peak.to_dict(orient="records")
            except Exception as e:
                peak_payload = {"error": str(e)}

            out_peak = EXPORT_BASE_DIR / "peak_hours_latest.json"
            write_json(out_peak, peak_payload, pretty=pretty)

            return {
                "route_suggestions_latest": str(out_full),
                "needs_data_latest": str(out_needs),
                "top_priority_latest": str(out_top),
                "summary_latest": str(out_summary),
                "peak_hours_latest": str(out_peak),
            }
